...
class SimpleDemo:
    ...
    def __init__(self,obstimes=None):
        ...
        for i,t in enumerate(self.obstimes):
            ...
            possibleobs = np.c_[self.stationlocations,vect] #we could have observations from either transmitter...
            obs = possibleobs[i%2,:] #we just select one transmitter then the other... repeat...
            self.observations.append(obs)
            self.trueLocations.append(location)
            self.trueAngles.append(vecToBearing(obs[2],obs[3]))
        
        #@Chris -- this doesn't make sense... why is there a second loop over locations at different times from the observations?
        ...
        self.trueAngles = np.array(self.trueAngles)
...
class SignalDemo(SimpleDemo):
    def __init__(self,bluetooth_experiments_path,burst_length = 1500,sample_interval = 100,sigs=None,obstimes=None):
        """
        ...
        if sigs is None: #TODO How to make this always find this file... TODO Make the path cross-platform (\ vs / etc).
            sigs = Signals(bluetooth_experiments_path+"/no rf amp experiments/noamploc2long.log",'d',angleOffset = 38)
