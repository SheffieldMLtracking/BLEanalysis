...
from scipy.stats import norm, circmean, entropy
...
class AnglesUseRejectionSampling(Angles):
    ...
    # Function to generate a record from some training data, consisting of samples taken close to the given times
    def GetSample(self, data, time):
        data_times = data[:, -1]
        times = self.burstTimes + time
        ...
        return signaldiffs, data[idxs, 2][0], sigs
        
    # Function to transform burst (generated by parseburst in signals.py) into a row to compare with the training record
    def GetSampleFromBurst(self, burst):
        ...
        # Use middle most packet for angle prediction
        return signaldiffs , data[idxs, 1][0], data[idxs, 0]

    def infer(self, inputBurst, rejectionThreshold = 15):
        self.rejectionThreshold = rejectionThreshold

        # Build table
        self.burstTimes = inputBurst['times'] - np.min(inputBurst['times'])
        
        mint = np.min(self.trainingdata[:, -1])
        maxt = np.max(self.trainingdata[:, -1])
    
        ts = np.arange(mint + 100e3, maxt, self.rejectionTableStep)
        n_rows = len(ts)
    
        signalstrengths, angle, _ = self.GetSample(self.trainingdata, ts[0])
        n_sig = len(signalstrengths)
    
        record = np.empty((n_rows, 1 + n_sig + 1), dtype=float)
    
        for i, t in enumerate(ts):
            signalstrengths, angle, _ = self.GetSample(self.trainingdata, t)
            record[i, 0] = t
            record[i, 1:1+n_sig] = signalstrengths
            record[i, -1] = angle

        # Sample from the table
        ss, ang, rawss = self.GetSampleFromBurst(inputBurst)

        mean_error = np.mean(np.abs(ss - record[:, 1:-1]), axis=1)
        selectedRecords = mean_error < rejectionThreshold

        #matchingrowindices = np.all(np.abs(training_data[:,:-1]-obs)<rejection_threshold,1)
        #selectedRecords = np.std((record[:,:-1]-ss)**2,1)<rejection_threshold

        selectedRecords = np.all(np.abs(ss - record[:, 1:-1]) < rejectionThreshold, 1) #all 5 measurements must be <6 dB out

        predangle = record[selectedRecords, -1] 
...
class OLDAnglesUseRejectionSampling(Angles):
    def __init__(self, rejectionTableStep = 50, trainingData = None):
        self.rejectionTableStep = rejectionTableStep
        if trainingData == None:
            self.trainingData = Signals("../bluetooth_experiments/no rf amp experiments/noamploc2long.log",'d',angleOffset = 38).data
